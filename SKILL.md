...
## 1 在你每次bootStrap的时候走下面的流程，启动你的个人办公室
...
关键: 在你自己的workspace里的项目下的根目录执行.


1.当你在任务开始前，你应该设置状态变为工作
...
2. 当你判断你目前在等待其他agent返回结果时，状态变为待命中

```bash
# 当你判断你目前在等待其他agent返回结果时，状态变为待命中
python3 set_state.py resting "待命中"

```

3. 当你判断你现在报错了，状态改为报错中 → 去 bug 区
```bash

# 当你判断你现在报错了，状态改为报错中 → 去 bug 区
python3 set_state.py error "发现问题，正在排查"

```
4. 当你判断你现在没有任何需要执行的任务，也没有其他需要等待返回的接口待命 → 回休息区
```bash

# 当你判断你现在没有任何的任务，也没有其他需要等待返回的接口待命 → 回休息区
